...
def register_user():
    ...
    global form_post, role, fname, req_role, lname, pss, email, tel
    form_post = 0
    ...
    if form_post:
        ...
        req_role = int(request.form.get('req_role'))
    else:
        try:
    ...
    if role == 'MANAGER':
        ...
        if req_role != 1 and req_role != 2 and req_role != 3 and req_role != 9:
            ...
            if form_post:
                return jsonify({'created': myresult[0][0]})
            else:
...
def get_all_users():
    mycursor = mysql.connection.cursor()
    global form_post, fname, lname, role
    form_post = 0
        ...
        if request.args.get('requested_role'):
            ...
            if fname and lname:
                sql_select_all_users = 'SELECT id, CONCAT(first_name, " ", last_name) name, role_id, email, ' \
                                       ...
                                       ' AND last_name = \'{1}\''.format(fname, lname)
            else:
...
def get_user(my_id):
    mycursor = mysql.connection.cursor()
    global form_post, author_id, role
    form_post = 0
...
def partial_modify_user(user_id):
    ...
    if role == 'MANAGER':
        ...
        if myresult:
            ...
            for i in range(0, len(bin5(bit_number))):
                if bin5(bit_number)[i] != '0':
                    ...
                    if i == 0:
                        values_list.append(myresult[0][1])
        ...
        output = 'You don\'t have the coresponding role; Your role is ' + role
        return jsonify({'Authentification failed! ': output}), 403
